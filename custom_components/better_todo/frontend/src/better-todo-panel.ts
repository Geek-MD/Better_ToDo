...
import type { HomeAssistant, HassEntity } from "./better-todo-shared.js";
...
// ── HA custom element property extensions ─────────────────────────────────
...
class BetterTodoPanel extends HTMLElement {
  ...
  private _showPane: boolean;
  private _initialized: boolean;
  private _mql: MediaQueryList | null;
  private _resizeObserver: ResizeObserver | null;
  private _entityListSignature: string;
  ...
  set narrow(value: boolean) {
    ...
    const layout = this.shadowRoot!.getElementById("layout") as HaElement | null;
    if (layout) layout.narrow = this._narrow;
    const menu = this.shadowRoot!.getElementById("menu") as HaElement | null;
    if (menu) menu.narrow = this._narrow;
  }
  ...
  private _init(entities: HassEntity[]): void {
    ...
    (root.getElementById("action-trigger") as HaElement).path = MDI_DOTS_VERTICAL;
    (root.getElementById("fab-icon") as HaElement).path = MDI_PLUS;
    (root.getElementById("dropdown-add-icon") as HaElement).path = MDI_PLUS;

    // Menu button
    const menu = root.getElementById("menu") as HaElement;
    ...
    const layout = root.getElementById("layout") as HaElement;
    ...
    root
      ...
      .addEventListener("wa-select", (ev: Event) => this._handleAction(ev as CustomEvent));
  ...
  private _syncPane(entities: HassEntity[]): void {
    ...
    list.querySelectorAll<HaElement>("[data-icon]").forEach((icon) => {
      const eid = icon.getAttribute("data-icon");
      const entity = entities.find((e) => e.entity_id === eid);
      if (entity) icon.stateObj = entity;
    });

    // Click handlers
    list.querySelectorAll<Element>("[data-entity]").forEach((item) => {
      item.addEventListener("click", () => {
        const eid = item.getAttribute("data-entity");
        if (eid) this._setEntity(eid);
      });
    });
  }
   ...
   */
  private _syncTitle(entities?: HassEntity[]): void {
    ...
    if (!this._showPane) {
      ...
      (titleSlot.querySelector("#title-chevron") as HaElement).path = MDI_CHEVRON_DOWN;

      titleSlot.querySelectorAll<HaElement>("[data-icon]").forEach((icon) => {
        const eid = icon.getAttribute("data-icon");
        const entity = ents.find((e) => e.entity_id === eid);
        if (entity) icon.stateObj = entity;
      });

      titleSlot.querySelectorAll<Element>("[data-entity]").forEach((item) => {
        item.addEventListener("click", () => {
          const eid = item.getAttribute("data-entity");
          if (eid) this._setEntity(eid);
        });
  ...
  private _syncCard(): void {
    const card = this.shadowRoot!.getElementById("workspace-card") as HaElement & {
      panel?: boolean;
      entityId?: string | null;
    } | null;
    if (card) {
    ...
    const fab = this.shadowRoot!.getElementById("fab") as HTMLElement | null;
    if (fab) {
      const hasEntity = Boolean(
        this._entityId && this._hass?.states?.[this._entityId]
      );
  ...
  private _syncMenu(): void {
    const menu = this.shadowRoot!.getElementById("menu") as HaElement | null;
    if (menu) menu.hass = this._hass ?? undefined;
  }

  // ── Event handlers ────────────────────────────────────────────────────

  private _handleMqlChange = (ev: MediaQueryListEvent): void => {
    this._mobile = ev.matches;
    if (this._initialized) this._syncTitle();
  };

  private _setEntity(entityId: string): void {
  ...
  private _handleAction(ev: CustomEvent): void {
    const action = (ev.detail as { item?: { value?: string } } | null)?.item?.value;
    if (action === "add") this._addItem();
  }
}
