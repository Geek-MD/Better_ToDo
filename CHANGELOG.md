...
### Fixed
- Await `panel_custom.async_register_panel(...)` to avoid the runtime warning `coroutine 'async_register_panel' was never awaited`.
- Ensure the Better To-do sidebar panel registration completes correctly so the custom panel can appear in Home Assistant.
