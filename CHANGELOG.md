...
### Fixed
- **Create-list reliability in panel**: aligned panel behavior with Home Assistant's `ha-panel-todo` permissions model by allowing list creation only for admin users, and hardened click handlers for *Create list* / *Back* actions to avoid context-binding issues in the custom Lit panel wrapper.
