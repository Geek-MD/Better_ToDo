...
### Fixed
- **`register_static_path` removed**: `async_setup` was calling the deprecated `hass.http.register_static_path()` which no longer exists in recent Home Assistant versions. Replaced with `hass.http.async_register_static_paths([StaticPathConfig(...)])`, the current API.
- **`CONFIG_SCHEMA` corrected**: replaced the raw `vol.Schema({}, extra=vol.ALLOW_EXTRA)` with `cv.empty_config_schema(DOMAIN)`, the idiomatic helper recommended by hassfest for integrations that use config entries and don't accept YAML configuration.
