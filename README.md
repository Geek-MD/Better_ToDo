...
- **Custom sidebar panel (HA To-do baseline)** – a dedicated *Better ToDo* panel at `/better-todo` using the same base structure, style, and interaction flow as Home Assistant's native To-do panel (two-pane layout, mobile list dropdown, native To-do list card rendering, and floating add-item action). List creation from this panel opens the `better_todo` config flow.
